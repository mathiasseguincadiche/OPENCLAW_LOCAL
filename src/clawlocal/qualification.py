...
def evaluate_benchmark(
    ...
    required_contexts = {
        int(value)
        for value in policy["required_contexts"]
    }
    ...
    for alias in required_models:
        ...
        checked = [
            case
            for case in selected
            if case.get("check_required", True)
        ]
        passed = [
            case
            for case in checked
            if case.get("check_passed") is True
        ]
        tps = [
        ...
        if missing_contexts:
            failures.append(
                f"contextes requis absents: {missing_contexts}"
            )
        ...
        if check_pass_rate < float(thresholds["min_check_pass_rate"]):
            failures.append(
                f"check_pass_rate={check_pass_rate:.3f}"
            )
        minimum_tps = float(
            thresholds["min_median_tokens_per_second"]
        )
        if median_tps is None or median_tps < minimum_tps:
            failures.append(
                f"median_tokens_per_second={median_tps}"
            )
        ...
        for context_text, minimum in per_context.items():
            ...
            context_cases = [
                case
                for case in checked
                if int(case.get("context", 0)) == context
            ]
            context_passed = [
                case
            ...
            if not context_cases or rate < float(minimum):
                failures.append(
                    f"context_{context}_check_pass_rate={rate:.3f}"
                )
