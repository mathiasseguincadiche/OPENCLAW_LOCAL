...
def _enforce_trigger_conditions(trigger: dict[str, object], *, local_web_attempted: bool, source_conflict_observed: bool, failure_evidence: bool, local_attempts: int, human_approved: bool) -> None:
    ...
    if max_local_attempts is not None and local_attempts < int(max_local_attempts):
        raise PermissionError(f"escalade refusée: {local_attempts} tentative(s) locale(s), {int(max_local_attempts)} requise(s)")
...
def select_route(agent: str, *, request_cloud: bool = False, cloud_enabled: bool = False, budget_ok: bool = False, reason: str | None = None, specialist_available: bool = False, deep_local_available: bool = False, local_web_attempted: bool = False, source_conflict_observed: bool = False, failure_evidence: bool = False, local_attempts: int = 0, human_approved: bool = False) -> RouteDecision:
    ...
    _enforce_trigger_conditions(trigger, local_web_attempted=local_web_attempted, source_conflict_observed=source_conflict_observed, failure_evidence=failure_evidence, local_attempts=local_attempts, human_approved=human_approved)
