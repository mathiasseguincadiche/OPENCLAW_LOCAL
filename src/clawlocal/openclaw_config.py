...
def _local_ref(alias: str, catalog: dict[str, Any]) -> str:
    ...
    raise ValueError(
        f"Le modèle {alias} utilise {provider}; import/backend et qualification explicites requis"
    )
...
def _agent_tools(agent_id: str, policy: dict[str, Any]) -> dict[str, Any]:
    ...
    tools: dict[str, Any] = {
        ...
        "elevated": {"enabled": bool(policy["security_defaults"]["elevated_enabled"])},
    }
        ...
        models.append(
            {
                ...
                "metadata": {"clawlocalAlias": alias, "status": model.get("status")},
            }
        )
    return models
...
def build_openclaw_patch(platform_root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "agents": {
            ...
            "defaults": {
                ...
                "model": {
                    ...
                    "fallbacks": [f"ollama/{catalog['models']['gemma-review']['runtime_id']}"],
                },
            },
            "entries": entries,
        },
        "tools": {
            ...
            "fs": {"workspaceOnly": bool(tool_policy["security_defaults"]["fs_workspace_only"])},
            "exec": {
                "mode": tool_policy["security_defaults"]["exec_mode"],
                "applyPatch": {"workspaceOnly": True},
            },
            "elevated": {"enabled": bool(tool_policy["security_defaults"]["elevated_enabled"])},
            "web": {
                "search": {
