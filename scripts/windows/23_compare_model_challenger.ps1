...
if ($DryRun) {
    Write-Host '[DRY-RUN] Comparaison obligatoire du spécialiste DevOps local V2.'
    Write-Host '[DRY-RUN] Incumbent : devstral-devops -> hf.co/mistralai/Ministral-3-14B-Reasoning-2512-GGUF:Q4_K_M.'
    Write-Host '[DRY-RUN] Challenger: granite-devops -> granite4.2:8b-q4_K_M.'
    ...
    Write-Host '[DRY-RUN] Granite reste hors routage nominal.'
...
if ($LASTEXITCODE -ne 0) {
    throw "Comparaison Ministral-Reasoning/Granite incomplète (code $LASTEXITCODE)."
}
