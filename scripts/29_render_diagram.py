...
def build_command(source: Path, output: Path) -> list[str]:
    ...
    if suffix in {".puml", ".plantuml"}:
        ...
        if output.stem != source.stem:
            raise ValueError("PlantUML conserve le nom de la source: utiliser le même stem pour la sortie")
    ...
    if suffix == ".dot":
        ...
        return [binary, f"-T{output_suffix.lstrip('.')}", str(source), "-o", str(output)]
