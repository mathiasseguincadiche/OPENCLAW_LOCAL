...
def build_command(source: Path, output: Path) -> list[str]:
    ...
    if output_suffix not in {".svg", ".png"}:
        raise ValueError("sortie attendue .svg ou .png")
    ...
    if suffix in {".puml", ".plantuml"}:
        ...
        if output.stem != source.stem:
            raise ValueError(
                "PlantUML conserve le nom de la source: "
                "utiliser le même stem pour la sortie"
            )
        fmt = "-tsvg" if output_suffix == ".svg" else "-tpng"
        return [binary, fmt, "-o", str(output.parent), str(source)]
    ...
    raise ValueError(
        "format source non supporté: utiliser .d2, .puml/.plantuml ou .dot"
    )


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Rend un diagramme-as-code localement."
    )
